...
def _args(argv: list[str], usage: str) -> tuple[Path, bool, str | None] | int:
    as_json = False
    only: str | None = None
    target = Path.cwd()
    rest = list(argv)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if job == "check":
        parsed = _args(argv[1:], CHECK_USAGE.format(checks=", ".join(CHECKS)))
        if isinstance(parsed, int):
            return parsed
        target, as_json, only = parsed
        if only and only not in CHECKS:
            return _err(f"unknown check: {only} (known: {', '.join(CHECKS)})")
        ...
        return report.render(run(cat, only), as_json)

    if job == "report":
        parsed = _args(argv[1:], REPORT_USAGE)
        if isinstance(parsed, int):
            return parsed
        target, as_json, _ = parsed
